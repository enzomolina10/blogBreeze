...
@section('content')
    <div class="max-w-4xl mx-auto py-6">
        <h1 class="text-3xl font-bold text-center mb-8 text-green-800">Editar Post</h1>

        <div class="bg-white p-6 rounded-lg shadow-lg">
            <form action="/category/{{ $post->id }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <input id="title"
                        ...
                        type="text" name="title" value="{{ $post->title }}" required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select id="category"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                        name="category">
                        <option value="Internacional" {{ $post->category == 'Internacional' ? 'selected' : '' }}>
                            Internacional</option>
                        <option value="Nacional" {{ $post->category == 'Nacional' ? 'selected' : '' }}>Nacional</option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="content"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                        name="content" rows="3" maxlength="255" required>{{ $post->content }}</textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input id="poster"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                        type="text" name="poster" value="{{ $post->poster }}" required>
                </div>
